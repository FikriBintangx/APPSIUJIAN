...
import id.ac.campus.antiexam.repository.LecturerRepository;
import id.ac.campus.antiexam.repository.SubjectRepository;
import id.ac.campus.antiexam.repository.StudentRepository;
import javax.swing.*;
...
public class AdminScheduleFrame extends JFrame {
    ...
    private final id.ac.campus.antiexam.repository.RoomRepository roomRepository = new id.ac.campus.antiexam.repository.RoomRepository();
    ...
    private JTextField txtAddPdfPath;
    ...
    private final Color COLOR_BORDER = Color.BLACK; // Thick Black Border
    ...
    private final Font FONT_ICON = new Font("Segoe UI Emoji", Font.PLAIN, 14); // Fix for square icons

    public AdminScheduleFrame(String adminUsername) {
    ...
    private JPanel createSidebar() {
        ...
        menuTitle.setForeground(new Color(224, 231, 255)); // Light Blue text
        menuTitle.setAlignmentX(Component.LEFT_ALIGNMENT);
        menuTitle.setBorder(new EmptyBorder(0, 10, 10, 0));
        menuPanel.add(menuTitle);

        sidebarButtons.clear();
        // Icons are emojis for simplicity in Neo theme or keep unicode
        btnMenuDashboard = createModernSidebarButton("Dashboard", "📊");
        btnMenuPenjadwalan = createModernSidebarButton("Penjadwalan", "📅");
        btnMenuDirectory = createModernSidebarButton("Direktori Data", "📂");
        btnMenuAbout = createModernSidebarButton("Tentang Aplikasi", "ℹ️");
        ...
        JButton btnQuickAdd = createModernActionButton("Tambah Jadwal", "➕", new Color(16, 185, 129));
        JButton btnQuickExport = createModernActionButton("Ekspor Data", "📤", new Color(245, 158, 11));
    ...
    // NEW: Modern sidebar button for Neo-Brutalism
    private JButton createModernSidebarButton(String text, String icon) {
        ...
        btn.setForeground(Color.WHITE); // Default Inactive Text
        btn.setContentAreaFilled(false);
        btn.setOpaque(false);

        btn.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) {
                // Hover effect? Maybe specific check if active
            }
        });
        return btn;
    }

    // NEW: Modern action button
    private JButton createModernActionButton(String text, String icon, Color color) {
        // Reuse createStyledButton logic effectively but with specific color
        JButton btn = createStyledButton(icon + " " + text, true, color);
        btn.setPreferredSize(new Dimension(200, 42)); // Fixed width for actions
        return btn;
    }
    ...
    private JPanel createTopHeader() {
        ...
        lblStatScheduled = new JLabel("0");
        lblStatAlert = new JLabel("0");

        statsPanel.add(createStatItem("Total", lblStatTotal, COLOR_PRIMARY));
        statsPanel.add(createStatItem("Berlangsung", lblStatOngoing, COLOR_SUCCESS));
        statsPanel.add(createStatItem("Terjadwal", lblStatScheduled, COLOR_WARNING));
        statsPanel.add(createStatItem("Peringatan", lblStatAlert, COLOR_ACCENT_RED));
    ...
    private JPanel createDashboardView() {
        ...
        statsCardsPanel.add(createStatCard("Total Ujian", lblCardTotal, "📊", COLOR_PRIMARY, "+12% dari bulan lalu"));
        statsCardsPanel.add(createStatCard("Sedang Berlangsung", lblCardOngoing, "🟢", COLOR_SUCCESS, "Hari ini"));
        statsCardsPanel
                .add(createStatCard("Terjadwal", lblCardScheduled, "📅", new Color(139, 92, 246), "7 hari ke depan"));
        statsCardsPanel.add(createStatCard("Peringatan", lblCardAlert, "⚠️", COLOR_ACCENT_RED, "Perlu perhatian"));

        RoundedPanel contentCard = new RoundedPanel(16, Color.WHITE);
        contentCard.setLayout(new BorderLayout());
        contentCard.setBorder(new EmptyBorder(0, 0, 0, 0));

        JPanel tableHeader = new JPanel(new BorderLayout());
        tableHeader.setBackground(Color.WHITE);
        tableHeader.setBorder(new CompoundBorder(
                new MatteBorder(0, 0, 2, 0, COLOR_BORDER),
                new EmptyBorder(20, 25, 20, 25)));

        JLabel tableTitle = new JLabel("📋 Jadwal Ujian Aktif");
        tableTitle.setFont(FONT_ICON.deriveFont(Font.BOLD, 18f));
        tableTitle.setForeground(COLOR_TEXT_HEADER);

        JPanel tableActions = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0));
        tableActions.setOpaque(false);

        JButton btnFilter = createTableActionButton("🔍 Filter", null, COLOR_BORDER, COLOR_TEXT_HEADER);
        JButton btnExport = createTableActionButton("📤 Ekspor", null, COLOR_BORDER, COLOR_TEXT_HEADER);
        JButton btnAdd = createTableActionButton("➕ Tambah Jadwal", null, COLOR_PRIMARY, Color.WHITE);

        btnExport.addActionListener(e -> showExportDialog());

        tableActions.add(btnFilter);
        tableActions.add(btnExport);
        tableActions.add(btnAdd);
        ...
        setupDashboardTableRenderers();

        // Filter Button Logic
        btnFilter.addActionListener(e -> {
            JPanel filterPanel = new JPanel(new GridLayout(2, 2, 10, 10));
            JComboBox<String> cbStatus = new JComboBox<>(new String[] { "All", "ONGOING", "SCHEDULED" });
            JComboBox<String> cbType = new JComboBox<>(new String[] { "All", "UTS", "UAS", "HARIAN", "REMEDIAL" });

            filterPanel.add(new JLabel("Status:"));
            filterPanel.add(cbStatus);
            filterPanel.add(new JLabel("Jenis Ujian:"));
            filterPanel.add(cbType);

            int result = JOptionPane.showConfirmDialog(this, filterPanel, "🔍 Filter Data",
            ...
            if (result == JOptionPane.OK_OPTION) {
                ...
                if (!"All".equals(status)) {
                    // Status is column 8 (index 8)
                    filters.add(RowFilter.regexFilter(status, 8));
                }
                if (!"All".equals(type)) {
                    // Type is column 5 (index 5)
        ...
        // Add action listener for buttons in table
        dashboardTable.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                int row = dashboardTable.rowAtPoint(e.getPoint());
                int col = dashboardTable.columnAtPoint(e.getPoint());

                // Column 10 is 'Aksi'
                if (row >= 0 && col == 10) {
                    // Calculate click position to distinguish Edit vs Delete
                    Rectangle cellRect = dashboardTable.getCellRect(row, col, false);
                    int x = e.getX() - cellRect.x;
                    int cellWidth = cellRect.width;

                    // Buttons are centered, approx 77px total width (36 + 5 + 36)
                    int contentWidth = 77;
                    int startX = (cellWidth - contentWidth) / 2;

                    // Check if clicked on Edit button (first 36px)
                    if (x >= startX && x <= startX + 36) {
                        try {
                            int id = Integer.parseInt(dashboardTable.getValueAt(row, 0).toString());

                            // Switch to Penjadwalan > Edit tab
                            switchView("VIEW_PENJADWALAN", btnMenuPenjadwalan);
                            JTabbedPane tabs = (JTabbedPane) ((JPanel) mainContentPanel.getComponent(1))
                                    .getComponent(1);
                            tabs.setSelectedIndex(2); // Edit & Hapus tab

                            // Find and select the row in examTableEdit
                            for (int i = 0; i < examTableEdit.getRowCount(); i++) {
                                ...
                                if (editId == id) {
                                    ...
                                    populateEditForm(); // Load data into form
                                    break;
                                }
                            }
                        } catch (Exception ex) {
                            System.err.println("Error editing: " + ex.getMessage());
                        }
                    }
                    // Check if clicked on Delete button (next 36px after 5px gap)
                    else if (x >= startX + 41 && x <= startX + 77) {
                        try {
                            int id = Integer.parseInt(dashboardTable.getValueAt(row, 0).toString());
                            int choice = JOptionPane.showConfirmDialog(
                                    AdminScheduleFrame.this,
                            ...
                            if (choice == JOptionPane.YES_OPTION) {
                                ...
                                JOptionPane.showMessageDialog(AdminScheduleFrame.this, "✅ Jadwal berhasil dihapus!");
                                loadAllData(); // Refresh tables
                            }
                        } catch (Exception ex) {
                            JOptionPane.showMessageDialog(AdminScheduleFrame.this, "❌ Gagal hapus: " + ex.getMessage());
                        }
                    }
                }
            }
        ...
        JScrollPane scroll = new JScrollPane(dashboardTable);
        scroll.setBorder(BorderFactory.createEmptyBorder());
        scroll.getViewport().setBackground(Color.WHITE);

        contentCard.add(tableHeader, BorderLayout.NORTH);
        contentCard.add(scroll, BorderLayout.CENTER);

        JPanel paginationPanel = new JPanel(new BorderLayout());
        paginationPanel.setBackground(Color.WHITE);
        paginationPanel.setBorder(new CompoundBorder(
                new MatteBorder(2, 0, 0, 0, COLOR_BORDER),
                new EmptyBorder(15, 25, 15, 25)));

        JLabel pageInfo = new JLabel("Menampilkan 1-10 dari 142 jadwal");
        pageInfo.setFont(FONT_BODY);
        pageInfo.setForeground(COLOR_TEXT_BODY);

        JPanel pageControls = new JPanel(new FlowLayout(FlowLayout.RIGHT, 5, 0));
        pageControls.setOpaque(false);

        JButton btnPrev = createPaginationButton("◀", false);
        JButton btn1 = createPaginationButton("1", true);
        JButton btn2 = createPaginationButton("2", false);
        JButton btn3 = createPaginationButton("3", false);
        JButton btnNext = createPaginationButton("▶", false);

        pageControls.add(btnPrev);
        pageControls.add(btn1);
        pageControls.add(btn2);
        pageControls.add(btn3);
        pageControls.add(btnNext);

        paginationPanel.add(pageInfo, BorderLayout.WEST);
        paginationPanel.add(pageControls, BorderLayout.EAST);

        contentCard.add(paginationPanel, BorderLayout.SOUTH);
    ...
    private JPanel createPenjadwalanView() {
        ...
        tabs.addTab("➕ Tambah Jadwal", createTabTambah());
        tabs.addTab("✏️ Edit & Hapus", createTabEdit());
        tabs.addTab("👥 Tambah Kelas", createTabTambahKelas());
        tabs.addTab("🏫 Tambah Ruangan", createTabTambahRuangan());
    ...
    private JPanel createTabTambah() {
        ...
        txtAddCode.setEditable(false);
        txtAddCode.setText(generateExamCode());
        txtAddTitle = new JTextField();
        cbAddRoom = new JComboBox<>();
        txtAddDuration = new JTextField("90");
        ...
        for (int i = 0; i < 60; i += 5)
            cbAddMinute.addItem(String.format("%02d", i));

        styleFormField(txtAddCode);
        styleFormField(txtAddTitle);
        styleComboBox(cbAddRoom);
        styleFormField(txtAddDuration);
        ...
        gbc.gridx = 0;
        gbc.gridy = 0;
        addFormRowSimple(form, gbc, "Kode Ujian", txtAddCode);
        gbc.gridx = 1;
        gbc.gridy = 0;
        ...
        gbc.gridx = 0;
        gbc.gridy = 2;
        addFormRowSimple(form, gbc, "Judul Ujian", txtAddTitle);
        gbc.gridx = 1;
        gbc.gridy = 2;
        addFormRowSimple(form, gbc, "Mata Kuliah (Kode)", cbAddCourse);
        ...
        lblTanggal.setForeground(COLOR_TEXT_HEADER);
        ...
        lblJam.setForeground(COLOR_TEXT_HEADER);
        form.add(lblJam, gbc);
        gbc.gridy++;
        ...
        gbc.gridx = 0;
        gbc.gridy = 8;
        addFormRowSimple(form, gbc, "⏱️ Durasi (Menit)", txtAddDuration);
        gbc.gridx = 1;
        gbc.gridy = 8;
        addFormRowSimple(form, gbc, "👨‍🏫 Pengajar (Username)", cbAddLecturer);

        gbc.gridx = 0;
        gbc.gridy = 10;
        // Removed Proctor and PDF selection
        ...
        JButton btnGenerateCode = createStyledButton("🔄 Reset Kode", false, COLOR_PRIMARY);
        btnGenerateCode.addActionListener(e -> txtAddCode.setText(generateExamCode()));
        JButton btnSave = createStyledButton("💾 Simpan Jadwal Baru", true, COLOR_PRIMARY);
        btnSave.addActionListener(e -> saveNewSchedule());
        btnPanel.add(btnGenerateCode);
    ...
    private JPanel createTabEdit() {
        ...
        scrollTable.setBorder(BorderFactory.createTitledBorder("📋 Pilih Jadwal untuk Diedit/Hapus"));

        JPanel form = new JPanel(new GridBagLayout());
        form.setBackground(Color.WHITE);
        form.setBorder(BorderFactory.createTitledBorder("✏️ Form Edit Data"));
        ...
        JButton btnUpdate = createStyledButton("💾 Simpan Perubahan", true, COLOR_PRIMARY);
        JButton btnDelete = createStyledButton("🗑️ Hapus Jadwal", false, COLOR_ACCENT_RED);
        JButton btnImport = createStyledButton("📥 Import Soal (Auto-Grade)", false, new Color(16, 185, 129));

        btnUpdate.addActionListener(e -> updateSchedule());
        btnDelete.addActionListener(e -> deleteScheduleFromEdit());
        btnImport.addActionListener(e -> importQuestionsForExam());
    ...
    private JPanel createDirectoryView() {
        ...
        headerCard.setLayout(new BorderLayout());
        headerCard.setBorder(new EmptyBorder(25, 30, 25, 30));
        JLabel lblTitle = new JLabel("👥 Direktori Dosen & Matkul");
        lblTitle.setFont(FONT_ICON.deriveFont(Font.BOLD, 24f));
        lblTitle.setForeground(COLOR_TEXT_HEADER);
        ...
        subjectModel = new DefaultTableModel(
                new Object[] { "ID", "Kode Matkul", "Nama Matkul", "Kelas", "Dosen Pengampu (User)" }, 0) {
            public boolean isCellEditable(int row, int column) {
                return false;
            }
        };
        ...
        JButton btnEditLect = createStyledButton("✏️ Edit Dosen", false, COLOR_PRIMARY);
        JButton btnDelLect = createStyledButton("🗑️ Hapus Dosen", false, COLOR_ACCENT_RED);

        JButton btnAddSub = createStyledButton("➕ Tambah Matkul", true, COLOR_PRIMARY);
        JButton btnEditSub = createStyledButton("✏️ Edit Matkul", false, COLOR_PRIMARY);
        JButton btnDelSub = createStyledButton("🗑️ Hapus Matkul", false, COLOR_ACCENT_RED);

        JButton btnAddStu = createStyledButton("➕ Tambah Mhs", true, COLOR_PRIMARY);
        JButton btnEditStu = createStyledButton("✏️ Edit Mhs", false, COLOR_PRIMARY);
        JButton btnDelStu = createStyledButton("🗑️ Hapus Mhs", false, COLOR_ACCENT_RED);
        ...
        // Dynamic Action Button Switching based on selected tab
    ...
    private JPanel createAboutView() {
        ...
        JLabel lblCredit = new JLabel("© 2025 Fikri Bintang Purnomo & Team");
    ...
    private JButton createTableActionButton(String text, String icon, Color bgColor, Color fgColor) {
        JButton btn = new JButton(text); // Icons are now in text directly (e.g. "🔍 Filter")
        ...
        btn.addMouseListener(new MouseAdapter() {
            ...
            public void mouseExited(MouseEvent e) {
                btn.setBackground(bgColor);
                btn.setBorder(new RoundedBorder(bgColor.darker(), 8));
            }
        });

        btn.setBorder(new RoundedBorder(bgColor.darker(), 8)); // Initial border
    ...
    private JButton createPaginationButton(String text, boolean active) {
        ...
        btn.setFont(text.equals("◀") || text.equals("▶") ? FONT_ICON.deriveFont(14f) : FONT_BODY);
    ...
    private JPanel createStatCard(String title, JLabel lblValue, String icon, Color color, String subtitle) {
        ...
        lblIcon.setFont(FONT_ICON.deriveFont(32f));
    ...
    private void setupDashboardTableRenderers() {
        // Sample data removed. Renderers setup below.

        dashboardTable.getColumnModel().getColumn(8).setCellRenderer(new DefaultTableCellRenderer() {
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value,
                    boolean isSelected, boolean hasFocus, int row, int column) {
                Component c = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
                if (value != null) {
                    ...
                    label.setBorder(BorderFactory.createCompoundBorder(
                            BorderFactory.createLineBorder(label.getBackground().darker()),
        ...
        dashboardTable.getColumnModel().getColumn(10).setCellRenderer(new DefaultTableCellRenderer() {
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value,
                    boolean isSelected, boolean hasFocus, int row, int column) {
                JPanel panel = new JPanel(new FlowLayout(FlowLayout.CENTER, 5, 0));
                panel.setOpaque(true);
                panel.setBackground(isSelected ? COLOR_TABLE_HOVER : Color.WHITE);

                JButton btnEdit = new JButton("✏️");
                btnEdit.setFont(new Font("Segoe UI", Font.PLAIN, 16));
                btnEdit.setPreferredSize(new Dimension(36, 36));
                btnEdit.setBackground(new Color(219, 234, 254));
                btnEdit.setForeground(COLOR_PRIMARY);
                ...
                btnDelete.setPreferredSize(new Dimension(36, 36));
                btnDelete.setBackground(COLOR_ACCENT_RED_LIGHT);
                btnDelete.setForeground(COLOR_ACCENT_RED);
                btnDelete.setBorder(BorderFactory.createCompoundBorder(
                        BorderFactory.createLineBorder(COLOR_ACCENT_RED_LIGHT.darker()),
                        BorderFactory.createEmptyBorder(5, 10, 5, 10)));
    ...
    private void loadAllData() {
        ...
        // Use TreeSet to keep classes sorted
        Set<String> classSet = new java.util.TreeSet<>();

        try {
            List<String> rooms = roomRepository.listRooms();
            for (String r : rooms) {
                cbAddRoom.addItem(r);
        ...
        try {
            List<Object[]> lecturers = lecturerRepository.listLecturers();
        ...
        try {
            List<Object[]> subjects = subjectRepository.listSubjects();
            subjectModel = new DefaultTableModel(new Object[] { "ID", "Kode", "Nama", "Kelas", "Dosen", "Pengawas" },
                    0); // Re-init with new col
            subjectTable.setModel(subjectModel); // Set new model
            // Hide ID column
            ...
            for (Object[] sub : subjects) {
                ...
                String className = (String) sub[3];

                cbAddCourse.addItem(code);
                cbEditCourse.addItem(code);
        ...
        try {
            if (studentModel != null) {
                List<Object[]> students = studentRepository.listStudents();
        ...
        // NEW: Load from students table as requested
        try (Connection conn = DBConnection.getConnection();
                PreparedStatement ps = conn.prepareStatement(
                        "SELECT DISTINCT class_name FROM students WHERE class_name IS NOT NULL AND class_name != '' ORDER BY class_name ASC")) {
        ...
        try {
            List<Object[]> exams = examRepository.listAllExamsForAdmin();
            ...
            for (Object[] row : exams) {
                total++;
                String status = (String) row[11]; // Status is at index 11
                ...
                // Map raw row to dashboard columns:
                // ID, Kode, Kelas, Judul, Matkul, Jenis, Durasi, Pengajar, Status, Waktu, Aksi
                dashboardModel.addRow(new Object[] {
                        ...
                        row[4], // Matkul
                        row[5], // Jenis
                        row[7] + " menit", // Durasi (add suffix for readability)
                        row[8], // Pengajar
                        row[11], // Status
                        row[12], // Waktu
                        "" // Aksi
            ...
            if (lblStatScheduled != null)
                lblStatScheduled.setText(String.valueOf(scheduled));
            if (lblStatAlert != null)
                lblStatAlert.setText(String.valueOf(alert));

            // Update Card Stats (Same logic)
            if (lblCardTotal != null)
                lblCardTotal.setText(String.valueOf(total));
            if (lblCardOngoing != null)
                lblCardOngoing.setText(String.valueOf(ongoing));
            if (lblCardScheduled != null)
                lblCardScheduled.setText(String.valueOf(scheduled));
            if (lblCardAlert != null)
                lblCardAlert.setText(String.valueOf(alert));

        } catch (Exception e) {
            System.err.println("Gagal load exam: " + e.getMessage());
        }

        // Final consolidated population of Class Combo
    ...
    private void saveClassData() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            JOptionPane.showMessageDialog(this, "✅ Data kelas dan siswa berhasil disimpan!");
            loadAllData();
    ...
    private void saveRoomData() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "✅ Ruangan berhasil disimpan!");
    ...
    private void saveNewSchedule() {
        try {
            ...
            int dur = Integer.parseInt(txtAddDuration.getText());
            String lectUser = (String) cbAddLecturer.getSelectedItem();

            // Proctor is explicitly null/empty now, to be assigned by Lecturer
            String procUser = "";

            // IMPORTANT: Ensure no open connections block this.
            // Although examRepository.createExam handles it, we should be careful.
            examRepository.createExam(code, cls, title, subjCode, type, fullTime, year, dur, lectUser, procUser, room);
            JOptionPane.showMessageDialog(this, "✅ Berhasil tambah jadwal! Tugas telah diberikan ke Dosen.");
            loadAllData();
            txtAddCode.setText(generateExamCode());
            txtAddTitle.setText("");
            txtAddDuration.setText("90");
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void updateSchedule() {
        ...
        try {
            ...
            examRepository.updateExam(selectedEditId, code, cls, title, subjCode, type, fullTime, year, dur, lectUser,
                    procUser, room);
    ...
    private void importQuestionsForExam() {
        if (selectedEditId == -1) {
        ...
        if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                // Process Import
                id.ac.campus.antiexam.service.QuestionImporter importer = new id.ac.campus.antiexam.service.QuestionImporter();
                importer.importFromFile(file, selectedEditId);

                setCursor(Cursor.getDefaultCursor());
                JOptionPane.showMessageDialog(this, "✅ Berhasil Mengimpor Soal ke Database!\n\n"
    ...
    private void deleteScheduleFromEdit() {
        ...
        int opt = JOptionPane.showConfirmDialog(this, "Hapus jadwal ID " + selectedEditId + "?", "🗑️ Konfirmasi",
                JOptionPane.YES_NO_OPTION);
        if (opt == JOptionPane.YES_OPTION) {
            try {
                examRepository.deleteExam(selectedEditId);
                loadAllData();
                selectedEditId = -1;
    ...
    private void openAddLecturerDialog() {
        ...
        int option = JOptionPane.showConfirmDialog(this, message, "➕ Tambah Dosen", JOptionPane.OK_CANCEL_OPTION);
    ...
    private void openEditLecturerDialog() {
        ...
        int option = JOptionPane.showConfirmDialog(this, message, "✏️ Edit Dosen", JOptionPane.OK_CANCEL_OPTION);
        if (option == JOptionPane.OK_OPTION) {
            try {
    ...
    private void deleteSelectedLecturer() {
        ...
        if (JOptionPane.showConfirmDialog(this, "Hapus dosen '" + username + "'?", "🗑️ Konfirmasi",
    ...
    private void openAddSubjectDialog() {
        // Auto-Generate Code internally or hide it
        // User requested: Name, Class (Dropdown), Lecturer (Dropdown), Proctor
        // (Dropdown)

        JTextField txtName = new JTextField();
        JComboBox<String> cbClass = new JComboBox<>();
        JComboBox<String> cbLect = new JComboBox<>();
        JComboBox<String> cbProctor = new JComboBox<>();

        // Populate Class from existing global list
        for (int i = 0; i < cbAddClass.getItemCount(); i++) {
            cbClass.addItem(cbAddClass.getItemAt(i));
        }

        // Populate Lecturers/Proctors from global list
        for (int i = 0; i < lecturerModel.getRowCount(); i++) {
            String uname = lecturerModel.getValueAt(i, 1).toString();
            cbLect.addItem(uname);
            cbProctor.addItem(uname);
        }

        Object[] message = {
                "Nama Matkul:", txtName,
                "Kelas:", cbClass,
                "Dosen (Pengajar):", cbLect,
                "Pengawas:", cbProctor
        };

        if (JOptionPane.showConfirmDialog(this, message, "➕ Tambah Matkul",
                JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
            try {
                String code = "MK-" + System.currentTimeMillis() % 100000; // Auto-generate simple code
                String selectedLecturer = (String) cbLect.getSelectedItem();
                String selectedProctor = (String) cbProctor.getSelectedItem();
                String selectedClass = (String) cbClass.getSelectedItem();

                subjectRepository.createSubject(code, txtName.getText(), selectedClass, selectedLecturer,
                        selectedProctor);
                loadAllData();
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "❌ Gagal: " + e.getMessage());
    ...
    private void openEditSubjectDialog() {
        ...
        JTextField txtName = new JTextField(subjectModel.getValueAt(row, 2).toString());

        // Class ComboBox
        JComboBox<String> cbClass = new JComboBox<>();
        String currentClass = subjectModel.getValueAt(row, 3) != null ? subjectModel.getValueAt(row, 3).toString() : "";
        for (int i = 0; i < cbAddClass.getItemCount(); i++) {
            cbClass.addItem(cbAddClass.getItemAt(i));
        }
        cbClass.setSelectedItem(currentClass);

        // Lecturer & Proctor
        JComboBox<String> cbLect = new JComboBox<>();
        JComboBox<String> cbProctor = new JComboBox<>();
        String currentLect = subjectModel.getValueAt(row, 4) != null ? subjectModel.getValueAt(row, 4).toString() : "";
        // Proctor is index 5 now after repo update, but table model might need
        // adjustment
        // in loadAllData if we want to show it.
        // For now, let's assume index 5 might be proctor if we update loadAllData, or
        // fetch from DB if needed.
        // Wait, I haven't updated loadAllData table model column!
        // To be safe, I'll update loadAllData logic too or just rely on what is
        // available.
        // Actually, let's look at listSubjects in repo, it returns proctor at index 5.
        // So I must update loadAllData to put it in table model.

        // Assuming loadAllData puts it in model, let's grab it.
        // But first, let's implement the dialog assuming data is there.
        String currentProctor = "";
        if (subjectModel.getColumnCount() > 5) { // Check if we updated model
            Object val = subjectModel.getValueAt(row, 5);
            if (val != null)
                currentProctor = val.toString();
        }

        for (int i = 0; i < lecturerModel.getRowCount(); i++) {
            String u = lecturerModel.getValueAt(i, 1).toString();
            cbLect.addItem(u);
            cbProctor.addItem(u);
        }
        cbLect.setSelectedItem(currentLect);
        cbProctor.setSelectedItem(currentProctor);
        ...
        if (JOptionPane.showConfirmDialog(this, message, "✏️ Edit Matkul",
                JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
            try {
                String uLect = (String) cbLect.getSelectedItem();
                String uProctor = (String) cbProctor.getSelectedItem();
                String uClass = (String) cbClass.getSelectedItem();

                subjectRepository.updateSubject(id, currentCode, txtName.getText(), uClass, uLect, uProctor);
                loadAllData();
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "❌ Gagal: " + e.getMessage());
            }
    ...
    private void deleteSelectedSubject() {
        ...
        if (row == -1)
            return;
        int id = Integer.parseInt(subjectModel.getValueAt(row, 0).toString());
        if (JOptionPane.showConfirmDialog(this, "Hapus matkul ini?", "🗑️ Konfirmasi",
                JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            try {
    ...
    private void openAddStudentDialog() {
        ...
        if (JOptionPane.showConfirmDialog(this, message, "➕ Tambah Mahasiswa",
                JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
            try {
    ...
    private void openEditStudentDialog() {
        ...
        if (JOptionPane.showConfirmDialog(this, message, "✏️ Edit Mahasiswa",
                JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
            try {
    ...
    private void deleteSelectedStudent() {
        ...
        if (JOptionPane.showConfirmDialog(this, "Hapus mahasiswa '" + name + "'?", "🗑️ Konfirmasi",
                JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            try {
    ...
    private void updateSidebarButtonState(JButton btn, boolean isActive) {
        if (isActive) {
            btn.setOpaque(true);
            btn.setBackground(Color.WHITE);
            btn.setForeground(COLOR_PRIMARY); // Active Text Blue
            btn.setFont(FONT_BODY_BOLD);
            // Simulate button shape if needed or just flat
        } else {
            btn.setOpaque(false);
            btn.setForeground(Color.WHITE); // Inactive Text White
    ...
    private void showExportDialog() {
        JDialog dialog = new JDialog(this, "📤 Ekspor Data Jadwal", true);
        ...
        JButton btnExcel = createStyledButton("📊 Ekspor ke Excel", true, new Color(34, 197, 94));
        JButton btnPdf = createStyledButton("🖨️ Cetak / Simpan PDF", true, COLOR_PRIMARY);
        ...
        JButton btnCancel = createStyledButton("Batal", false, Color.GRAY);
    ...
    private void exportToExcel() {
        ...
        fileChooser.setSelectedFile(new java.io.File("Laporan_Jadwal_Ujian_" + System.currentTimeMillis() + ".xls"));
        ...
        if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try (java.io.PrintWriter writer = new java.io.PrintWriter(fileChooser.getSelectedFile())) {
                ...
                html.append("<div class='title'>Laporan Jadwal Ujian - SiUjian</div>");
                html.append("<table>");

                // Headers
                html.append("<thead><tr>");
                for (int i = 0; i < examModelList.getColumnCount(); i++) {
                    html.append("<th>").append(examModelList.getColumnName(i)).append("</th>");
                }
                html.append("</tr></thead>");

                // Rows
                html.append("<tbody>");
                for (int i = 0; i < examModelList.getRowCount(); i++) {
                    html.append("<tr>");
                    for (int j = 0; j < examModelList.getColumnCount(); j++) {
                        Object val = examModelList.getValueAt(i, j);
                        String s = val == null ? "-" : val.toString();
                        // formatting status with color
                        if (j == 8) { // Status column
                            String color = "#000";
                            if ("ONGOING".equalsIgnoreCase(s))
                                color = "#16A34A"; // Green
                            else if ("SCHEDULED".equalsIgnoreCase(s))
                                color = "#2563EB"; // Blue
                            else if ("FINISHED".equalsIgnoreCase(s))
                                color = "#DC2626"; // Red
                            html.append("<td style='color:").append(color).append("; font-weight:bold;'>").append(s)
                                    .append("</td>");
                        } else {
    ...
    private void printToPDF() {
        try {
            java.text.MessageFormat header = new java.text.MessageFormat("Laporan Jadwal Ujian (SiUjian)");
            java.text.MessageFormat footer = new java.text.MessageFormat("Halaman {0,number,integer}");

            // Print the full list table
            boolean complete = examTableList.print(JTable.PrintMode.FIT_WIDTH, header, footer, true, null, true, null);
            if (complete) {
    ...
    private void choosePdfFile() {
        ...
        if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            txtAddPdfPath.setText(chooser.getSelectedFile().getAbsolutePath());
        }
    }
            ...
            try (java.io.BufferedReader br = new java.io.BufferedReader(new java.io.FileReader(file))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (parts.length >= 2) {
                        ...
                        // Basic validation to skip header if present
                ...
                pnlStudentInputs.revalidate();
                pnlStudentInputs.repaint();
                JOptionPane.showMessageDialog(this, "✅ Berhasil import " + count + " data siswa dari CSV.");
            } catch (Exception e) {
