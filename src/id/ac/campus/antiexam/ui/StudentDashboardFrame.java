...
public class StudentDashboardFrame extends JFrame {
    ...
    private final id.ac.campus.antiexam.repository.SessionRepository sessionRepo = new id.ac.campus.antiexam.repository.SessionRepository();
    ...
    private void loadStudentProfile() {
        try {
            ...
            String sql = "SELECT name, student_number, class_name FROM students WHERE id = ?";
            ...
            if (rs.next()) {
                ...
                studentClass = rs.getString("class_name");
            } else {
    ...
    private void loadStudentExams() {
        ...
        try {
            Connection conn = DBConnection.getConnection();
            String sql = "SELECT e.id AS exam_original_id, se.id AS student_exam_id, e.title AS exam_title, e.code AS exam_code, e.target_class, e.room, e.scheduled_at, e.status, s.name AS subject_name "
                    + "FROM exams e "
                    + "LEFT JOIN student_exams se ON e.id = se.exam_id AND se.student_id = ? "
                    + "LEFT JOIN subjects s ON e.subject_code = s.code "
                    + "WHERE (e.target_class = ? OR e.status = 'ONGOING') "
                    + "ORDER BY e.status, e.scheduled_at DESC";
    ...
    private void initUI() {
        ...
        // Dot Pattern Background for content
    ...
    private JPanel createSidebar() {
        ...
        sidebar.setBorder(BorderFactory.createMatteBorder(0, 0, 0, 4, Color.BLACK)); // Thick border
        ...
        JLabel lblFooter = new JLabel("© 2025 SiUjian v2.0");
    ...
    private JPanel createContentPanel() {
        ...
        btnRefresh.addActionListener(e -> {
            loadStudentExams();
            content.remove(1); // Remove old grid/empty panel (index 1)
    ...
    private JPanel createCourseCard(CourseItem item, String icon) {
        ...
        JLabel lblCode = new JLabel(item.examCode != null ? item.examCode : "-");
        ...
        JLabel lblExam = new JLabel(subLine);
        lblExam.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        lblExam.setForeground(MUTED_TEXT);
        ...
        // Add minimal border to status badge
    ...
    private void showTokenDialog(CourseItem course) {
        ...
        btnSubmit.addActionListener(e -> {
            ...
            // Validate Token against Database (examCode is loaded from DB)
            if (!token.equalsIgnoreCase(course.examCode)) {
                javax.swing.JOptionPane.showMessageDialog(dialog, "Token Salah! Harap minta token valid dari Dosen.",
            ...
            // Auto-Join / Create Session if first time
            if (course.studentExamId == 0) {
                try {
                    // Use specific exam creation
                    int newSessionId = sessionRepo.createSessionForExam(course.examOriginalId, studentId);
    ...
    private void openExamFrame(CourseItem course, String token) {
        SwingUtilities.invokeLater(() -> {
            ...
            StudentExamFrame examFrame = new StudentExamFrame(course.studentExamId, name, token);
