...
public class LoginFrame extends JFrame {
    ...
    public LoginFrame() {
        ...
        card.setPreferredSize(new Dimension(1100, 650)); // 900x550 → 1100x650 BIGGER!
        ...
        try {
            ...
            Image img = icon.getImage().getScaledInstance(180, 180, Image.SCALE_SMOOTH); // 120 → 180px GEDEIN!
            lblLogo.setIcon(new ImageIcon(img));
            lblLogo.setHorizontalAlignment(SwingConstants.CENTER);
        } catch (Exception e) {
            lblLogo.setText("👨‍🎓");
            lblLogo.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 120)); // 72 → 120px
        }

        JLabel lblHero = new JLabel("SiUjian");
        lblHero.setFont(new Font("Segoe UI", Font.BOLD, 42)); // 32 → 42px
        ...
        lblDesc.setFont(new Font("Segoe UI", Font.PLAIN, 16)); // 14 → 16px
        lblDesc.setForeground(new Color(71, 85, 105));

        GridBagConstraints gbcL = new GridBagConstraints();
        gbcL.gridx = 0;
        gbcL.gridy = 0;
        gbcL.insets = new Insets(0, 0, 30, 0); // 20 → 30px
        contentLeft.add(lblLogo, gbcL);
        gbcL.gridy = 1;
        gbcL.insets = new Insets(15, 0, 15, 0); // 10 → 15px
        ...
        rightPanel.setBorder(new EmptyBorder(60, 70, 60, 70)); // 50,60 → 60,70
    ...
    private void performLogin() {
        ...
        if (name.isEmpty() || nim.isEmpty()) {
            lblError.setText("⚠️ Nama dan NIM wajib diisi.");
            return;
        }
        ...
        try {
            ...
            if (studentData != null) {
                ...
                lblError.setText("⚠️ Data tidak ditemukan. Periksa kembali Nama dan NIM.");
            }
        } catch (Exception ex) {
            lblError.setText("⚠️ Terjadi kesalahan sistem.");
            ex.printStackTrace();
        }
    }
